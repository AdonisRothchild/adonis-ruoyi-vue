...
public class AutoFillObjectHandler implements MetaObjectHandler
{
    ...
    private void setDefaultValue(MetaObject metaObject, FieldFill fillType)
    {
        ...
        //找到所有有DataFill注解的属性
        ReflectionUtils
                .doWithFields(metaObject.getOriginalObject().getClass(), needFillList::add, getFieldFilter(fillType));
        ...
        for (Field field : needFillList)
        {
            ...
            } else
            {
                throw new RuntimeException(StringUtils
                        .format("默认值填充错误,{}的类型应该为{},传入的类型与其不兼容", field.getName(), fill.fillClass().getName()));
            }
        }
    }
    ...
    private FieldFilter getFieldFilter(FieldFill fillType)
    {
        return field ->
        {
            ...
            return (fill != null && tableField.fill().equals(fillType)) || FieldFill.INSERT_UPDATE
                    .equals(tableField.fill());
        };
    }
}
